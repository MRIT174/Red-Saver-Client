...
const Profile = () => {
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `https://red-saver-server.vercel.app/users/${user.email}`, // single slash
          {
            headers: {
  ...
  // Update filtered districts based on selected division
  useEffect(() => {
  ...
  if (!user || loading) return <p className="p-6 text-center">Loading profile...</p>;
      ...
      <div className="bg-white shadow rounded-xl p-6 flex flex-col md:flex-row gap-6 items-center md:items-start">
        <div className="avatar">
          <div className="w-32 rounded-full ring ring-red-500 ring-offset-2">
            <img
              src={dbUser?.avatar || user?.photoURL || "/images/default-avatar.png"}
              alt="User Avatar"
            />
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="mt-4 text-gray-700">
            <p>
              <span className="font-semibold">Division:</span> {dbUser.division || "Not set"}
            </p>
            <p>
              <span className="font-semibold">District:</span> {dbUser.district || "Not set"}
            </p>
          </div>
